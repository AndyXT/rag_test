...
def restore_backup(backup_path: Path, db_path: str) -> bool:
    ...
    try:
        ...
        if final_path.exists():
            current_backup = Path(
                str(final_path) + f"_before_restore_{int(datetime.now().timestamp())}"
            )
...
def create_manual_backup(db_path: str) -> bool:
    ...
    try:
        ...
        backup_path = Path(
            str(source_path) + f"_manual_backup_{int(datetime.now().timestamp())}"
        )
...
def main():
    ...
    parser.add_argument(
        "--db-path", type=str, default="./chroma_db", help="Path to ChromaDB database"
    )
    ...
    cleanup_parser.add_argument(
        "--keep", type=int, default=5, help="Number of backups to keep (default: 5)"
    )
