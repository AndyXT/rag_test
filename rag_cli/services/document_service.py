...
class DocumentService:
    ...
    def get_document_stats(docs_path: str) -> Dict[str, Any]:
        ...
        """
        path = Path(docs_path)
        stats = {"total_files": 0, "total_size_mb": 0, "files": []}
        ...
        for pdf_file in pdf_files:
            ...
            stats["files"].append({"name": pdf_file.name, "size_mb": round(size_mb, 2)})
    ...
    def check_disk_space(path: str, required_mb: float = 500) -> Tuple[bool, str]:
        ...
        try:
            ...
            if available_mb < required_mb:
                return (
                    False,
                    f"Insufficient disk space. Need {required_mb}MB, have {available_mb:.0f}MB",
                )
