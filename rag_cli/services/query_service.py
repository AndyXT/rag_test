...
class QueryService:
    ...
    async def process_query(
        self, question: str, use_rag: bool = True, temperature: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _should_use_rag(self) -> bool:
        ...
        return (
            self.rag_system.vectorstore is not None
            and self.rag_system.qa_chain is not None
        )
    ...
    async def _execute_rag_query(self, question: str) -> Dict[str, Any]:
        ...
        future = loop.run_in_executor(self.executor, self._execute_rag_sync, question)
            ...
            if self.rag_system.qa_chain:
                ...
                return {
                    ...
                    "metadata": {
                        ...
                        "reranking_used": self.rag_system.settings_manager.get(
                            "use_reranker", False
                        ),
                        "query_expansion_used": self.rag_system.settings_manager.get(
                            "use_query_expansion", False
                        ),
                        "query_refinement_used": self.rag_system.settings_manager.get(
                ...
                retrieval_k = self.rag_system.settings_manager.get(
    ...
    async def _execute_direct_query(self, question: str) -> Dict[str, Any]:
        ...
        future = loop.run_in_executor(
            self.executor, self._execute_direct_sync, question
        )
    ...
    def _expand_query_if_enabled(self, query: str) -> str:
        ...
        if self.rag_system.settings_manager.get("use_query_refinement", False):
            if self.rag_system.query_expansion_llm:  # Same LLM used for refinement
                return self.query_processor.refine_query(
                    query, self.rag_system.query_expansion_llm
                )
        ...
        # Otherwise check if query expansion is enabled
            ...
            if self.rag_system.query_expansion_llm:
                return self.query_processor.expand_query(
                    query, self.rag_system.query_expansion_llm
                )

        return query

    def _rerank_if_enabled(self, query: str, docs: List[Document]) -> List[Document]:
        ...
        if min_score > 0:
            reranked_docs = self.query_processor.filter_documents_by_score(
                reranked_docs, min_score
            )
    ...
    def _process_rag_result(
        self, response: str, docs: List[Document], context: str
    ) -> Dict[str, Any]:
