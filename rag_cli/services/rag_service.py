...
class RAGService:
    ...
    def __init__(self, settings_file: str = "settings.json"):
        ...
        model_name = self.settings_manager.get(
            "ollama_model", self.settings_manager.get("model_name", DEFAULT_MODEL)
        )
        ...
        chunk_overlap = self.settings_manager.get(
        ...
        self.database_service = DatabaseService(
            self.rag_system.vectorstore_manager, self.settings_manager
        )
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            settings = {
                "model_name": model_name,
                "temperature": self.settings_manager.get(
                    "temperature", DEFAULT_TEMPERATURE
                ),
                "chunk_size": self.settings_manager.get(
            ...
            RichLogger.info(
                f"RAG Service initialized with model: {settings['model_name']}"
            )
    ...
    async def process_query(
        ...
        try:
            ...
            result = await self.query_service.process_query(
                question=question, use_rag=use_rag, temperature=temperature
            )
            ...
            if save_to_history:
                self.chat_service.add_assistant_message(
                    content=error_result["answer"], method="error"
                )
    ...
    def create_database(
        ...
        success = self.database_service.create_database(
            docs_path=docs_path, db_path=db_path, progress_callback=progress_callback
        )
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        return {
            "settings": {
                ...
                "use_query_expansion": self.settings_manager.get(
    ...
    def export_chat_history(
        self, session_id: Optional[str] = None, format: str = "markdown"
    ) -> str:
        """
        Export chat history
