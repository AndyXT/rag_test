...
class RerankerManager:
    ...
    def initialize(self) -> bool:
        ...
        if not CROSSENCODER_AVAILABLE:
            RichLogger.warning(
                "CrossEncoder not available. Install sentence-transformers to use reranking."
            )
            return False

        self._reranker_model = self.settings_manager.get(
    ...
    def rerank_documents(
        ...
        try:
            ...
            if top_k is None:
                top_k = (
                    self.settings_manager.get("reranker_top_k", 3)
    ...
    def _predict_with_timeout(
        self, pairs: List[List[str]], timeout: float
    ) -> Optional[List[float]]:
        """
