...
class RAGSystem:
    ...
    def __init__(
        ...
        self.query_executor = QueryExecutor(
            self.llm_manager, self.query_processor, self.error_handler
        )
        self.retrieval_manager = RetrievalManager(
            self.vectorstore_manager, self.query_processor
        )
    ...
    def create_db_from_docs(
        ...
        self.vectorstore_manager.create_db_from_docs(
            docs_path, db_path, progress_callback
        )
    ...
    def _expand_query(self, original_query: str) -> List[str]:
        ...
        return self.query_processor.expand_query(
            original_query, self.query_expansion_llm
        )
    ...
    def _create_expanded_retriever(self, k: int) -> BaseRetriever:
        ...
        return self.retrieval_manager.create_expanded_retriever(
            k, self.query_expansion_llm
        )
    ...
    async def _execute_qa_chain(self, question: str) -> Dict[str, Any]:
        ...
        return await self.query_executor.execute_qa_chain(
            self.qa_chain, question, self._rerank_documents
        )
    ...
    async def _execute_manual_retrieval(self, question: str) -> Dict[str, Any]:
        ...
        return await self.query_executor.execute_manual_retrieval(
            retriever, question, self._rerank_documents, system_prompt
        )
    ...
    async def query(self, question: str) -> Dict[str, Any]:
        ...
        if not self.vectorstore:
            return {
                "response": "RAG system not initialized. Load or create a database first.",
                "context": [],
            }
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, self._execute_query_in_thread, question
            )
            return result
    ...
    def _handle_query_error(self, error: Exception) -> Dict[str, Any]:
        ...
        return {"response": user_message, "context": [], "error_info": error_info}
