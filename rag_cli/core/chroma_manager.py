...
class ChromaManager:
    ...
    def load_existing_db(self, db_path: str, embeddings: Any) -> bool:
        ...
        try:
            ...
            if "fds_to_keep" in str(e):
                RichLogger.warning(
                    "💡 Try restarting the application or increasing file descriptor limit"
                )
            return False
    ...
    def _create_temp_database(
        self, texts: List[Document], embeddings: Any, batch_size: int
    ) -> Tuple[Chroma, Path]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            vectorstore = self._process_batch(
                vectorstore, batch, embeddings, str(temp_path), batch_size
            )
        ...
        try:
            ...
            if batch_size > 5:
                return self._retry_with_smaller_batches(
                    vectorstore, batch, embeddings, persist_dir, batch_size
                )
            return vectorstore
    ...
    def _finalize_database(
        self, temp_path: Path, final_path: Path, backup_path: Optional[Path]
    ) -> None:
    ...
    def _restore_backup_on_failure(
        self, backup_path: Optional[Path], final_path: Path
    ) -> None:
